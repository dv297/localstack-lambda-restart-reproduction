...
resource "aws_api_gateway_rest_api" "api_gateway_rest_api" {
  ...
  body = templatefile("${local.src_path}/openapi.yaml", {
    minimal_service_arn    = module.minimal_service.lambda_function_arn
    lambda_a_arn           = module.lambda_a.lambda_function_arn
    lambda_b_arn           = module.lambda_b.lambda_function_arn
    lambda_c_arn           = module.lambda_c.lambda_function_arn
...
data "aws_iam_policy_document" "lambda_invoke_policy_document" {
  statement {
    ...
    resources = [
      module.lambda_a.lambda_function_arn,
      module.lambda_b.lambda_function_arn,
      module.lambda_c.lambda_function_arn,
    ]
  }
}
...
module "minimal_service" {
  ...
  publish       = true
  function_name = "${local.resource_prefix}-minimal_service"
  ...
  source_path = [
    ...
    {
      path             = "${local.src_path}/services/minimal_service"
  ...
  s3_existing_package = var.is_localstack_deploy ? {
    ...
    key = "${abspath(local.project_root)}/src/services/minimal_service"
  } : null
}
...
module "lambda_a" {
  source        = "terraform-aws-modules/lambda/aws"
  publish       = true
  function_name = "${local.resource_prefix}-service_a"
  handler       = "app.lambda_handler"
  runtime       = "python3.11"
  timeout       = 900
  ...
  source_path = [
    ...
    {
      path             = "${local.src_path}/services/service_a"
      pip_requirements = true
    }
  ]
  ...
  s3_existing_package = var.is_localstack_deploy ? {
    bucket = "hot-reload"
    # To enable hot reloading, this must be an absolute path
    key = "${abspath(local.project_root)}/src/services/service_a"
  } : null
}
...
module "lambda_b" {
  source        = "terraform-aws-modules/lambda/aws"
  publish       = true
  function_name = "${local.resource_prefix}-service_b"
  handler       = "app.lambda_handler"
  runtime       = "python3.11"
  timeout       = 900
  ...
  source_path = [
    ...
    {
      path             = "${local.src_path}/services/service_b"
      pip_requirements = true
    }
  ]
  ...
  s3_existing_package = var.is_localstack_deploy ? {
    bucket = "hot-reload"
    # To enable hot reloading, this must be an absolute path
    key = "${abspath(local.project_root)}/src/services/service_b"
  } : null
}
...
module "lambda_c" {
  source        = "terraform-aws-modules/lambda/aws"
  publish       = true
  function_name = "${local.resource_prefix}-service_c"
  handler       = "app.lambda_handler"
  runtime       = "python3.11"
  timeout       = 900
  ...
  source_path = [
    ...
    {
      path             = "${local.src_path}/services/service_c"
      pip_requirements = true
    }
  ]
  ...
  s3_existing_package = var.is_localstack_deploy ? {
    bucket = "hot-reload"
    # To enable hot reloading, this must be an absolute path
    key = "${abspath(local.project_root)}/src/services/service_c"
  } : null
}
